<template>
  <footer class="footer">
      <div class="level">
        <div class="level-left">
          <div class="level-item social">
            <a href="https://twitter.com/KalenderJawaDev" title="Bot Kalender Jawa 🤖 di Twitter">          
              <i class="fab fa-twitter fa-lg"></i>
            </a>
            <a href="https://github.com/kalenderjawa" alt="Kalender Jawa Github" title="Source Code di Github 🚀" target="_blank" rel="noopener"> 
              <i class="fab fa-github fa-lg"></i>
            </a>
            <!---
            <a href="https://join.slack.com/t/kalender-jawa/shared_invite/enQtNzM3MjA1OTM3OTg5LWExZTEyYTZhYWQ5NGRkOWU0NGZiZjk1Y2ZhY2M5N2E3NWUyYjI0Y2YwNDEyOTFiZTMwYTgxZjEyNzE1YjZiMzk"
            title="Kalau mau diskusi di Slack yuk... :)">
              <i class="fab fa-slack fa-lg"></i>
            </a>
            -->
          </div>
        </div>
        <div class="level-right">
          <div class="level-item">
            <span><a href="https://kalenderjawa.dev">Pustaka Kalender Jawa</a></span>
          </div>
          <div class="level-item">
            <span><a href="https://app.kalenderjawa.dev" class="app-link">Sengkala</a></span>
          </div>
          <div class="level-item">
            <span> © 2019 - 2021</span>
          </div>
          <div class="level-item">
            <span><a href="https://sajen.id" class="grad-link">Equan Pr</a> & Kolaborator</span>
          </div>
        </div>
      </div>
  </footer>  
</template>

<script>
export default {
  name: 'Footer'
}
</script>
